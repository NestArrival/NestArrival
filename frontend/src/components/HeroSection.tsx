...
export default function HeroSection({ user: propUser, loading: propLoading }: HeroSectionProps) {
  ...
  return (
    <>
      <section
        ref={heroRef}
        className="relative pt-8 pb-48 sm:pb-40 md:pb-56 lg:pb-64 px-0 sm:px-6 lg:px-8 z-10 overflow-visible"
      >
        {/* Background video */}
        ...
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center justify-between relative z-10 gap-4 lg:gap-12 pb-4 md:pb-0 overflow-visible px-4 sm:px-0">
          ...
          <div className="lg:w-1/2 space-y-6 relative z-20 text-center lg:text-left mt-2">
            ...
            <p
              ref={subheadingRef}
              className="text-base text-[#5c544d] max-w-xl mx-auto lg:mx-0 leading-relaxed opacity-0"
            >
            ...
            >
              <Link
                href="/signup"
                className="bg-[#cfa052] hover:bg-[#b58942] text-white px-8 py-4 rounded-full font-bold shadow-[0_8px_20px_rgba(207,160,82,0.3)] hover:shadow-[0_15px_30px_rgba(207,160,82,0.5)] hover:-translate-y-1 transition-all duration-300 flex items-center gap-2 group text-sm uppercase tracking-wide"
              >
                Start Your Journey
        ...
        {(!loading && user?.role === "OWNER") ? (
          <div
            ref={searchBarRef}
            className="opacity-0 mt-8 md:mt-0 md:absolute md:left-1/2 md:-translate-x-1/2 md:-bottom-12 w-full md:w-[90%] md:max-w-5xl md:z-30 mx-auto px-4 md:px-0"
          >
          ...
          <div
            ref={searchBarRef}
            className="opacity-0 mt-8 md:mt-0 md:absolute md:left-1/2 md:-translate-x-1/2 md:-bottom-12 w-full md:w-[90%] md:max-w-5xl md:z-30 mx-auto px-4 md:px-0 overflow-visible"
          >
            <div className="relative z-30 bg-white rounded-2xl sm:rounded-3xl shadow-[0_20px_50px_rgba(44,39,36,0.08)] border border-[#eae1d3] p-4 md:p-3">

              {user?.role !== "TENANT" && (
                <div className="flex flex-col sm:flex-row sm:items-center border-b border-[#eae1d3] px-2 sm:px-4 pb-2 gap-3 sm:gap-6">
                  <button
                    onClick={() => setSearchRole("Tenant")}
                    className={`text-xs font-bold pb-2 border-b-2 transition-colors cursor-pointer ${searchRole === "Tenant" ? "border-[#cfa052] text-[#cfa052]" : "border-transparent text-[#8a7d6a] hover:text-[#2c2724]"}`}
                  >
                    Find a Home
                  </button>
                  <button
                    onClick={() => setSearchRole("Owner")}
                    className={`text-xs font-bold pb-2 border-b-2 transition-colors cursor-pointer ${searchRole === "Owner" ? "border-[#cfa052] text-[#cfa052]" : "border-transparent text-[#8a7d6a] hover:text-[#2c2724]"}`}
                  >
                    List a Property
                  </button>
                </div>
                ...
                <div className="flex-1 w-full md:border-r border-[#eae1d3] px-2 py-1 flex items-center gap-3 min-h-[48px] md:min-h-[64px]">
                  ...
                  <div className="w-full">
                    ...
                    <input
                      ...
                      className="w-full text-sm font-bold text-[#2c2724] outline-none placeholder:text-[#a89e8d] bg-transparent"
                    />
                  </div>
                </div>
      ...
      <div className="h-6 md:h-24 bg-[#f4efe6]" />
    </>
  );
}