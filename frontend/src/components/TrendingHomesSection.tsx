    ...
    <section className="py-32 px-4 sm:px-6 lg:px-8 bg-[#f4efe6]" aria-busy={!!propLoading}>
      <div className="max-w-6xl mx-auto space-y-14">
        <div className="text-center max-w-3xl mx-auto space-y-4">
          <span className="text-[10px] text-[#cfa052] font-extrabold uppercase tracking-widest block">
            Trending Homes You&apos;ll Love
          </span>
          <h2 className="font-serif text-3xl sm:text-4xl font-bold text-[#2c2724] tracking-tight">
            Explore Spaces Designed for Your Next Chapter.
          </h2>
          <p className="text-sm text-[#5c544d] max-w-2xl mx-auto leading-relaxed">
            NestArrival helps international tenants discover verified living
            spaces that match their lifestyle, budget, comfort, and relocation
            goals before arriving in a new country.
          </p>
        </div>
        ...
        >
                ...
                <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
                  <div className="space-y-2">
                    ...
                    <div className="border-t border-[#f4efe6] pt-3 grid grid-cols-2 gap-y-2 text-[10px] text-[#5c544d]">
                      <div>
                        <span className="text-[#8a7d6a]">Furnishing:</span>{" "}
                        <strong className="text-[#2c2724] block mt-0.5">
                          {listing.furnishing}
                        </strong>
                      </div>
                      <div>
                        <span className="text-[#8a7d6a]">Transit:</span>{" "}
                        <strong className="text-[#2c2724] block mt-0.5">
                          {listing.transit}
                        </strong>
                      </div>
                      <div className="col-span-2">
                        <span className="text-[#8a7d6a]">Move-in Date:</span>{" "}
                        <strong className="text-[#2c2724] block mt-0.5">
                          {listing.moveIn}
                        </strong>
                      </div>
                  ...
                  <div className="space-y-3 pt-2">
                    <div className="flex flex-wrap gap-1.5">
                      {listing.tags.map((tag) => (
                        <span
                          key={tag}
                          className="text-[9px] font-extrabold bg-[#f4efe6] text-[#8a7d6a] px-2 py-0.5 rounded-md"
                        >
                          {tag}
                        </span>
                      ))}
          ...
          <motion.div variants={fadeUpItem} className="h-full">
            <TiltCard
              tiltMax={3}
              className="h-full min-h-[400px] w-full rounded-[2rem] border border-[#eae1d3] bg-[#fdfbf7] shadow-[0_12px_30px_rgba(44,39,36,0.03)] p-8 flex flex-col justify-center items-center text-center"
            >
              ...
              <h3 className="font-serif text-xl font-black text-[#2c2724] mb-2">
                Looking for more?
              </h3>
              <p className="text-[11px] text-[#5c544d] max-w-xs mb-6">
                Create a verified profile to unlock hundreds of exclusive,
                title-vetted properties globally.
              </p>
              <Link
                href={user ? "/tenant/dashboard" : "/login"}
                className="text-xs font-bold text-[#cfa052] border-b border-[#cfa052] pb-0.5 hover:text-[#b58942] hover:border-[#b58942] transition-colors"
              >
                View All Properties
              </Link>
            </TiltCard>
